...
public class PayEveryoneClient implements ClientModInitializer {
	...
	public void onInitializeClient() {
		// Clear player lists on game start/restart
		...
		stopPaymentKey = KeyBindingHelper.registerKeyBinding(VersionCompat.createKeyMapping(
		...
		// Clear player lists when joining a server
		ClientPlayConnectionEvents.JOIN.register((handler, sender, client) -> {
			PayManager.getInstance().clearAllPlayerLists();
		});
		
		// Clear player lists when disconnecting (for rejoin)
